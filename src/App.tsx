...
export default function App() {
  ...
  if (focusedPingId) {
    ...
    return <main className={`app-shell notification-shell${focused ? ` type-${focused.type}` : ""}`}><header className="app-header"><div className="brand-lockup"><img src="/icon.svg" alt="" /><h1>{focused?.status === "completed" && focused.type !== "message" ? "Results" : "New Ping"}</h1></div></header><div className="content">{focused ? renderCards([focused], "This Ping is no longer available.", true) : <StatusPanel title="Ping unavailable" message="This Ping may have been deleted." />}</div></main>;
  }

  return <main className="app-shell">
    <header className="app-header"><button type="button" className="brand-lockup brand-button" aria-label="Open inbox" onClick={() => setView("inbox")}><img src="/icon.svg" alt="" /><h1>{view === "inbox" ? "Ping" : view === "create" ? "Create Ping" : "Settings"}</h1></button><div className="header-actions"><button type="button" className={`header-icon create-icon${view === "create" ? " active" : ""}`} aria-label="Create Ping" aria-pressed={view === "create"} onClick={() => { setPrefill(null); setView(view === "create" ? "inbox" : "create"); }}><CreateGlyph /></button><button type="button" className={`header-icon settings-icon${view === "settings" ? " active" : ""}`} aria-label="Settings" aria-pressed={view === "settings"} onClick={() => setView(view === "settings" ? "inbox" : "settings")}><GearGlyph /></button><a className="help-link" href="https://obr-ping.ex-asperis.com/" target="_blank" rel="noreferrer" aria-label="Open Ping help">?</a></div></header>
