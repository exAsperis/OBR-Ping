...
import { getNotificationPreference, setNotificationPreference, type NotificationPreference } from "../preferences";
...
export function SettingsPanel({ role, settings, pings, metadata, onChanged }: Props) {
  ...
  return <div className="stack">
    <section className="panel stack compact"><span className="eyebrow">This device</span><h2>Incoming Pings</h2><label>Notification behavior<select value={preference} onChange={(event) => changePreference(event.target.value as NotificationPreference)}><option value="badge-toast">Badge + toast</option><option value="badge">Badge only</option><option value="auto-open">Automatically open Ping</option></select></label><p className="muted">This setting is stored only in this browser.</p></section>
    {role === "GM" ? <>
